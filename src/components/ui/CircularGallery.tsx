...
function createTextTexture(gl, text, summary = '', font = 'bold 42px Figtree', color = 'white', isMobile = false) {
  ...
  context.quadraticCurveTo(btnX + btnWidth, btnY, btnX + btnWidth, btnY + r);
  context.lineTo(btnX + btnWidth, btnY + btnHeight - r);
  context.quadraticCurveTo(btnX + btnWidth, btnY + btnHeight, btnX + btnWidth - r, btnY + btnHeight);
  context.lineTo(btnX + r, btnY + btnHeight);
  context.quadraticCurveTo(btnX, btnY + btnHeight, btnX, btnY + btnHeight - r);
  context.lineTo(btnX, btnY + r);
  context.quadraticCurveTo(btnX, btnY, btnX + r, btnY);
  context.closePath();
  context.fill();
...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = this.plane.scale.y; // Match the card height exactly for 1:1 overlay
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = sin(p.x * 2.0) * abs(uSpeed) * 0.1;
      ...
      fragment: `
        ...
        void main() {
          ...
          // Simplified color sampling (Removing lines/sheen/aberration)
...
class App {
  ...
  createCamera() {
    ...
    this.camera.fov = 45;
    this.camera.position.z = 25; // Moved back to prevent clipping
  }
  createScene() {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.galleryItems = items && items.length ? items : defaultItems;
    this.mediasImages = this.galleryItems.concat(this.galleryItems);
  ...
  onTouchMove(e) {
    ...
    // Inverted distance to follow natural hand movement
...
export default function CircularGallery({
  ...
  return (
    <div className="circular-gallery-container w-full h-full relative group">
      <div className="circular-gallery" ref={containerRef} />
      
      {/* Navigation Buttons */}
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 z-30 pointer-events-none px-4 md:px-12 flex justify-between">
        <button 
          onClick={handlePrev}
          className="pointer-events-auto w-12 h-12 md:w-16 md:h-16 rounded-full border border-white/10 bg-black/20 backdrop-blur-md flex items-center justify-center text-white transition-all duration-300 hover:bg-[#837FFB] hover:border-[#837FFB] hover:scale-110 opacity-0 group-hover:opacity-100"
        >
